...
if(!function_exists('get_detheme_number')){
    ...
    function get_detheme_number(){
       ...
       if ( ! file_exists( $theme) )
             return esc_html__( 'Stylesheet is missing.','hue');
...
if( !class_exists( 'DethemeReduxFramework' ) ) {
    ...
    class DethemeReduxFramework {
            ...
            self::$_properties = array( 
                'args' => array(
                    ...
                    'menu_title' => array( 
                            ...
                            'default'=> esc_html__( 'Options', 'redux-framework' )
                        ),              
                    'page_title' => array( 
                            'data_type'=>'string', 
                            'label'=>'Page Title', 
        ...
        public function __render( $sections = array(), $args = array(), $extra_tabs = array() ) {
            ...
            $defaults['last_tab']           = '0';
            $defaults['menu_icon']          = '';
            $defaults['menu_title']         = esc_html__( 'Options', 'redux-framework' );
            $defaults['page_icon']          = 'icon-themes';
            $defaults['page_title']         = esc_html__( 'Options', 'redux-framework' );
            ...
            $defaults['footer_credit']      = '<span id="footer-thankyou">' . esc_html__( 'Options panel created using', 'redux-framework') . ' <a href="' . esc_url($this->framework_url) . '" target="_blank">' . esc_html__('Redux Framework', 'redux-framework') . '</a> v' . self::$_version . '</span>';
            $defaults['help_tabs']          = array();
            $defaults['help_sidebar']       = ''; // esc_html__( '', 'redux-framework' );
...
/**
                    ...
                    foreach( $section['fields'] as $fieldk => $field ) {
                        if( isset( $field['type'] ) && $field['type'] != "callback"  ) {
                            ...
                            if( !class_exists( $field_class ) ) {
                                ...
                                if( $class_file && file_exists(get_template_directory().'/'.$class_file) ) {
                                    /** @noinspection PhpIncludeInspection */


                                    require_once( get_template_directory().'/'.$class_file );
                                }
                            }   
        ...
        public function _enqueue() {
           ...
           wp_register_style(
            ...
            wp_register_script( 
                'select2-js', 
                self::$_url . 'assets/js/vendor/select2/select2.js',
                array( 'jquery' ),
                null,
                true
            );
            ...
            $this->localize_data['args'] = array(
                ...
                'reset_section_confirm' => esc_html__( 'Are you sure? Resetting will lose all custom values in this section.', 'redux-framework' ), 
        ...
        public function _register_extensions() {     
            ...
            foreach($folders as $folder){
                ...
                if( !class_exists( $extension_class ) ) {
                    ...
                    if(file_exists(get_template_directory().'/'.$class_file)) {
                        /** @noinspection PhpIncludeInspection */
                        require_once(get_template_directory().'/'.$class_file);
                        /** @noinspection PhpUnusedLocalVariableInspection */
                ...
                if( isset( $section['fields'] ) ) {
                    foreach( $section['fields'] as $field ) {
                        ...
                        elseif(isset( $field['validate_callback'] ) && $field['id']=='detheme_license' && !function_exists( $field['validate_callback'] )){
                            ...
                            //$field['msg']=__('L'. 'i' .'c'. 'e'. 'n'. 's'. 'e'. ' '.'n'. 'u'. 'm'.'b'.'e'.'r'.' n'.'o'.'t'.' '.'v'.'a'.'l'.'i'.'d'.','.' '.'p'.'l'.'e'.'a'.'s'.'e'.' '.'t'.'r'.'y'.' '.'a'.'g'.'a'.'i'.'n'.'.','hue');    
                            $field['msg']=__('License number not valid, please try again','hue');    
                            $this->errors[] = $field;                     
                        }
        ...
        public function _options_page_html() {
            ...
            echo '<a href="javascript:void(0);" id="expand_options">' . esc_html__( 'Expand', 'redux-framework' ) . '</a>';
            echo '<div class="redux-action_bar">';
            submit_button( esc_html__( 'Save Changes', 'redux-framework'), 'primary', 'redux_save', false );
            echo '&nbsp;';
            submit_button( esc_html__( 'Reset Section', 'redux-framework'), 'secondary', $this->args['opt_name'] . '[defaults-section]', false );
            echo '&nbsp;';            
            submit_button( esc_html__( 'Reset All', 'redux-framework' ), 'secondary', $this->args['opt_name'] . '[defaults]', false );
            echo '</div>';

            echo '<div class="redux-ajax-loading" alt="' . esc_html__( 'Working...', 'redux-framework' ) . '">&nbsp;</div>';
            echo '<div class="clear"></div>';
            echo '</div>';
            ...
            if( isset( $_GET['settings-updated'] ) && $_GET['settings-updated'] == 'true' && $saved == '1' ) {
                if( isset( $this->options['REDUX_imported'] ) && $this->options['REDUX_imported'] === 1 ) {
                    echo '<div id="redux-imported">' . apply_filters( 'redux-imported-text-' . $this->args['opt_name'], '<strong>' . esc_html__( 'Settings Imported!', 'redux-framework' ) ) . '</strong></div>';
                } else {
                    echo '<div id="redux-save">' . apply_filters( 'redux-saved-text-' . $this->args['opt_name'], '<strong>'.esc_html__( 'Settings Saved!', 'redux-framework' ) ) . '</strong></div>';
                }
            }

            echo '<div id="redux-save-warn">' . apply_filters( 'redux-changed-text-' . $this->args['opt_name'], '<strong>'.esc_html__( 'Settings have changed, you should save them!', 'redux-framework' ) ) . '</strong></div>';
            echo '<div id="redux-field-errors"><strong><span></span> ' . esc_html__( 'error(s) were found!', 'redux-framework' ) . '</strong></div>';
            echo '<div id="redux-field-warnings"><strong><span></span> ' . esc_html__( 'warning(s) were found!', 'redux-framework' ) . '</strong></div>';
            ...
            if( $this->args['show_import_export'] === true ) {
                ...
                if( !empty( $this->args['icon_type'] ) && $this->args['icon_type'] == 'image' ) {
                    $icon = ( !isset( $this->args['import_icon'] ) ) ? '' : '<img src="' . esc_url($this->args['import_icon']) . '" /> ';
                } else {
                ...
                echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="redux-group-tab-link-a" data-rel="import_export_default">' . $icon . ' <span class="group_title">' . esc_html__( 'Import / Export', 'redux-framework' ) . '</span></a>';
                echo '</li>';
            ...
            if( $this->args['dev_mode'] === true ) {
                ...
                if( !empty( $this->args['icon_type'] ) && $this->args['icon_type'] == 'image' ) {
                    $icon = ( !isset( $this->args['dev_mode_icon'] ) ) ? '' : '<img src="' . esc_url($this->args['dev_mode_icon']) . '" /> ';
                } else {
            ...
            if( $this->args['system_info'] === true ) {
                ...
                if( !empty( $this->args['icon_type'] ) && $this->args['icon_type'] == 'image' ) {
                    $icon = ( !isset( $this->args['system_info_icon'] ) ) ? '' : '<img src="' . esc_url($this->args['system_info_icon']) . '" /> ';
                } else {
                ...
                echo '<a href="javascript:void(0);" id="system_info_default_section_group_li_a" class="redux-group-tab-link-a custom-tab" data-rel="system_info_default">' . $icon . ' <span class="group_title">' . esc_html__( 'System Info', 'redux-framework' ) . '</span></a>';
                echo '</li>';
            }
            ...
            if( $this->args['show_import_export'] === true ) {
                ...
                echo '<h3>' . esc_html__( 'Import / Export Options', 'redux-framework' ) . '</h3>';
                echo '<h4>' . esc_html__( 'Import Options', 'redux-framework' ) . '</h4>';
                echo '<p><a href="javascript:void(0);" id="redux-import-code-button" class="button-secondary">' . esc_html__( 'Import from file', 'redux-framework' ) . '</a> <a href="javascript:void(0);" id="redux-import-link-button" class="button-secondary">' . esc_html__( 'Import from URL', 'redux-framework' ) . '</a></p>';

                echo '<div id="redux-import-code-wrapper">';

                echo '<div class="redux-section-desc">';
                echo '<p class="description" id="import-code-description">' . apply_filters( 'redux-import-file-description', esc_html__( 'Input your backup file below and hit Import to restore your sites options from a backup.', 'redux-framework' ) ) . '</p>';
                echo '</div>';
                ...
                echo '<div class="redux-section-desc">';
                echo '<p class="description" id="import-link-description">' . apply_filters( 'redux-import-link-description', esc_html__( 'Input the URL to another sites options set and hit Import to load the options from that site.', 'redux-framework' ) ) . '</p>';
                echo '</div>';
                ...
                echo '<p id="redux-import-action"><input type="submit" id="redux-import" name="' . $this->args['opt_name'] . '[import]" class="button-primary" value="' . esc_html__( 'Import', 'redux-framework' ) . '">&nbsp;&nbsp;<span>' . apply_filters( 'redux-import-warning', esc_html__( 'WARNING! This will overwrite all existing option values, please proceed with caution!', 'redux-framework' ) ) . '</span></p>';

                echo '<h4>' . esc_html__( 'Export Options', 'redux-framework' ) . '</h4>';
                echo '<div class="redux-section-desc">';
                echo '<p class="description">' . apply_filters( 'redux-backup-description', esc_html__( 'Here you can copy/download your current option settings. Keep this safe as you can use it as a backup should anything go wrong, or you can use it to restore your settings on this site (or any other site).', 'redux-framework' ) ) . '</p>';
                echo '</div>';

                /** @noinspection PhpUndefinedConstantInspection */
                echo '<p><a href="javascript:void(0);" id="redux-export-code-copy" class="button-secondary">' . esc_html__( 'Copy', 'redux-framework' ) . '</a> <a href="' . esc_url(add_query_arg( array( 'feed' => 'reduxopts-' . $this->args['opt_name'], 'action' => 'download_options', 'secret' => md5( AUTH_KEY . SECURE_AUTH_KEY ) ), site_url() )) . '" id="redux-export-code-dl" class="button-primary">' . esc_html__( 'Download', 'redux-framework' ) . '</a> <a href="javascript:void(0);" id="redux-export-link" class="button-secondary">' . esc_html__( 'Copy Link', 'redux-framework' ) . '</a></p>';
...
<div id="message" class="updated">
    <p><strong><?php printf(esc_html__( 'Your website using %s Theme', 'hue' ),$item_name); ?></strong></p>
    <p><?php esc_html_e('Please enter your Item Purchase Code to get theme & plugin update notification','hue');?></p>
    <p class="submit">
     <a href="<?php echo esc_url(admin_url('themes.php?page='.$this->args['page_slug'])); ?>&tab=info" class="register-button"><?php esc_html_e( 'Register Template', 'hue' ); ?></a> | 
     <a href="<?php print esc_url(add_query_arg( 'detheme_notification_'.strtolower($item_name), 'hide_'.strtolower($item_name).'_detheme_notification' )); ?>" target="_parent"><?php esc_html_e( 'Hide this notice', 'hue' );?></a>
</a>
    </p>
</div>
        <?php
