...
/* 
Plugin Name: Hue Demo Installer
Plugin URI: http://www.detheme.com/
Description: Import all the detheme demo content and theme settings
Version: 1.0.0
Author: detheme.com
Author URI: http://www.detheme.com/
Domain Path: /languages/
Text Domain: hue-demo
 */
...
class hue_one_click_demo{
	...
	function __construct(){
		...
		if(!defined('DETHEME_ONE_URI')){
			define('DETHEME_ONE_URI',plugins_url( '/', __FILE__ ));
		} 
	...
	function init(){
   	...
   	function load_detheme_demo(){
		...
		load_plugin_textdomain('hue-demo', false, dirname(plugin_basename(__FILE__)) . '/languages/');

	    add_action( 'admin_menu', array($this,'register_submenu_page'));
   		add_filter( 'import_option_accepted',array($this,'import_option_accepted'));
		add_filter( 'import_post_meta_key', array( $this, 'is_valid_meta_key' ) );
		add_filter( 'import_term_meta_key', array( $this, 'is_valid_meta_key' ) );
   	}
   	...
   	function do_install($package_name){
		...
		$install_ess_grid=false;
 		...
 		$packages=hue_one_click_demo::get_packages();
		...
		$spectech_path=$path."spectech-field.json";
		...
		if(! (bool)$skip_content){
			...
			if($wp_filesystem->exists( $spectech_path) && is_plugin_active('detheme-spectech/spectech.php')){
				...
				if(!class_exists('WP_Spectech_Field_Import')){
					require_once(SPECTECH_DIR."lib/importer.class.php");
				}

				$this->install_spectech_fields($spectech_path);
	   		} 
		}
...
*/
	...
	function install_spectech_fields($json_file){

		$datas=WP_Spectech_Field_Import::import_from_file( $json_file );
	...
	function import_start( $file ) {
		if ( ! is_file($file) ) {
			echo '<p><strong>' . esc_html__( 'Sorry, there has been an error.', 'hue-demo' ) . '</strong><br />';
			esc_html_e( 'The file does not exist, please try again.', 'hue-demo' ) . '</p>';
			die();
		}
		...
		if ( is_wp_error( $import_data ) ) {
			echo '<p><strong>' . esc_html__( 'Sorry, there has been an error.', 'hue-demo' ) . '</strong><br />'.
			esc_html( $import_data->get_error_message(),'hue-demo' ) . '</p>';
			die();
		}
	...
	function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			$term_meta = isset( $cat['category_termmeta'] ) ? (array) $cat['category_termmeta'] : array();
			...
			if ( ! is_wp_error( $id ) ) {
				...
				if(get_option( 'db_version' ) >= 34370  && count($term_meta)){

					foreach ($term_meta as $meta) {
						$key = apply_filters( 'import_term_meta_key', $meta['key'], $id, $cat );
						$value = false;
				...
				printf( esc_html__( 'Failed to import category %s', 'hue-demo' ), esc_html($cat['category_nicename']) );
	...
	function process_tags() {
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				print '<div class="updated error">';
				printf( esc_html__( 'Failed to import post tag %s', 'hue-demo' ), esc_html($tag['tag_name']) );
				if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG )
					echo ': ' . $id->get_error_message();
				echo '<br />';
				print '</div>';
	...
	function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			$term_meta = isset( $term['term_meta'] ) ? (array) $term['term_meta'] : array();
			...
			if ( ! is_wp_error( $id ) ) {
				if ( isset($term['term_id']) ){
					...
					if(get_option( 'db_version' ) >= 34370  && count($term_meta)){
						...
						foreach ($term_meta as $meta) {
							$key = apply_filters( 'import_term_meta_key', $meta['key'], $id['term_id'], $term );
							$value = false;
				...
				print '<div class="updated error">';
				printf( esc_html__( 'Failed to import %s %s', 'hue-demo' ), esc_html($term['term_taxonomy']), esc_html($term['term_name']) );
				if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG )
					echo ': ' . $id->get_error_message();
				echo '<br />';
				print '</div>';
	...
	function process_posts() {
		...
		foreach ( $this->posts as $post ) {
			...
			if ( ! post_type_exists( $post['post_type'] ) ) {
				print '<div class="updated error">';
				printf( esc_html__( 'Failed to import &#8220;%s&#8221;: Invalid post type %s', 'hue-demo' ),
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				print '<div class="updated error">';
				printf( esc_html__('%s &#8220;%s&#8221; already exists.', 'hue-demo'), $post_type_object->labels->singular_name, esc_html($post['post_title']) );
				echo '<br />';
				print '</div>';
				...
				if ( is_wp_error( $post_id ) ) {
					print '<div class="updated error">';
					printf( esc_html__( 'Failed to import %s &#8220;%s&#8221;', 'hue-demo' ),
				...
				foreach ( $post['terms'] as $term ) {
					...
					if ( ! $term_id ) {
						...
						if ( ! is_wp_error( $t ) ) {
							...
							print '<div class="updated error">';
							printf( esc_html__( 'Failed to import %s %s', 'hue-demo' ), esc_html($taxonomy), esc_html($term['name']) );
	...
	function process_menu_item( $item ) {
		...
		if ( ! $menu_slug ) {
			esc_html_e( 'Menu item skipped due to missing menu slug', 'hue-demo' );
			echo '<br />';
			return;
		}
		...
		if ( ! $menu_id ) {
			printf( esc_html__( 'Menu item skipped due to invalid menu slug: %s', 'hue-demo' ), esc_html( $menu_slug ) );
			echo '<br />';
			return;
		} else {
...
*/
/*
		print_r($item['postmeta'][1]['key']);
		print_r('<br />');
		print_r($item['postmeta']);
		print_r('<br />');

		$_menu_item_type = $item['postmeta'][1]['value'];
*/
/*
		$_menu_item_type 				= $item['postmeta'][1]['value'];
		$_menu_item_menu_item_parent 	= $item['postmeta'][2]['value'];
		$_menu_item_object_id 			= $item['postmeta'][3]['value'];
		$_menu_item_object 				= $item['postmeta'][4]['value'];
		$_menu_item_target 				= $item['postmeta'][5]['value'];
		$_menu_item_classes 			= $item['postmeta'][6]['value'];
		$_menu_item_xfn 				= $item['postmeta'][7]['value'];
		$_menu_item_url 				= $item['postmeta'][8]['value'];
		$_menu_item_dt_megamenu_description 					= $item['postmeta'][9]['value'];
		$_menu_item_dt_megamenu_background_url 					= $item['postmeta'][10]['value'];
		$_menu_item_dt_megamenu_background_horizontal_position 	= $item['postmeta'][11]['value'];
		$_menu_item_dt_megamenu_background_vertical_position 	= $item['postmeta'][12]['value'];
		$_menu_item_dt_megamenu_background_repeat 				= $item['postmeta'][13]['value'];
		$_menu_item_dt_megamenu_width_options 					= $item['postmeta'][14]['value'];
		$_menu_item_dt_megamenu_width 							= $item['postmeta'][15]['value'];
...
/*
		$_menu_item_type = ($item['postmeta']['_menu_item_type']) ? $item['postmeta']['_menu_item_type'] : '';
		$_menu_item_menu_item_parent = ($item['postmeta']['_menu_item_menu_item_parent']) ? $item['postmeta']['_menu_item_menu_item_parent'] : 0;
		$_menu_item_classes = ($item['postmeta']['_menu_item_classes']) ? $item['postmeta']['_menu_item_classes'] : '';
		$_menu_item_object_id = ($item['postmeta']['_menu_item_object_id']) ? $item['postmeta']['_menu_item_object_id'] : 0;
		$_menu_item_object = ($item['postmeta']['_menu_item_object']) ? $item['postmeta']['_menu_item_object'] : '';
		$_menu_item_url = ($item['postmeta']['_menu_item_url']) ? $item['postmeta']['_menu_item_url'] : '';
		$_menu_item_target = ($item['postmeta']['_menu_item_target']) ? $item['postmeta']['_menu_item_target'] : '';
		$_menu_item_xfn = ($item['postmeta']['_menu_item_xfn']) ? $item['postmeta']['_menu_item_xfn'] : '';
*/
		...
		foreach ( $item['postmeta'] as $meta ) {
			switch ($meta['key']) {
				...
				case '_menu_item_url':
					...
					break;
				case '_menu_item_dt_megamenu_description':
					$_menu_item_dt_megamenu_description = $meta['value'];
					break;
				case '_menu_item_dt_megamenu_background_url':
					$_menu_item_dt_megamenu_background_url = $meta['value'];
					break;
				case '_menu_item_dt_megamenu_background_horizontal_position':
					$_menu_item_dt_megamenu_background_horizontal_position = $meta['value'];
					break;
				case '_menu_item_dt_megamenu_background_vertical_position':
					$_menu_item_dt_megamenu_background_vertical_position = $meta['value'];
					break;
				case '_menu_item_dt_megamenu_background_repeat':
					$_menu_item_dt_megamenu_background_repeat = $meta['value'];
					break;
				case '_menu_item_dt_megamenu_width_options':
					$_menu_item_dt_megamenu_width_options = $meta['value'];
					break;
				case '_menu_item_dt_megamenu_width':
					$_menu_item_dt_megamenu_width = $meta['value'];
					break;
		...
		if(isset($_menu_item_dt_megamenu_content_pages)){
			$args['menu_item_dt_megamenu_content_pages']=$_menu_item_dt_megamenu_content_pages;
		}
	...
	function proccess_post_custom_meta($menu_id, $menu_item_db_id, $args){
		...
		if(isset($args['menu_item_dt_megamenu_content_pages'])){
				update_post_meta( $menu_item_db_id, '_menu_item_dt_megamenu_content_pages', sanitize_key($args['menu_item_dt_megamenu_content_pages']) );
		}
	...
	function process_attachment( $post, $url ) {
		...
		if ( $info = wp_check_filetype( $upload['file'] ) )
			...
			return new WP_Error( 'attachment_processing_error', esc_html__('Invalid file type', 'hue-demo') );
	...
	function fetch_remote_file( $url, $post ) {
		...
		if ( ! $headers ) {
			...
			return new WP_Error( 'import_file_error', $url." :".esc_html__('Remote server did not respond', 'hue-demo') );
		}

		if ( $headers['response'] != '200' ) {
			@unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', sprintf( esc_html__('Remote server returned error response %1$d %2$s', 'hue-demo'), esc_html($headers['response']), get_status_header_desc($headers['response']) ) );
		}
		...
		if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
			$wp_filesystem->delete($upload['file']);
			return new WP_Error( 'import_file_error', esc_html__('Remote file is incorrect size', 'hue-demo') );
		}

		if ( 0 == $filesize ) {
			$wp_filesystem->delete($upload['file']);
			return new WP_Error( 'import_file_error', esc_html__('Zero size file downloaded', 'hue-demo') );
		}

		$max_size = (int) $this->max_attachment_size();
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			$wp_filesystem->delete($upload['file']);
			return new WP_Error( 'import_file_error', sprintf(esc_html__('Remote file is too large, limit is %s', 'hue-demo'), size_format($max_size) ) );
		}
	...
	function register_submenu_page(){
		 ...
		 if($packages=hue_one_click_demo::get_packages()){
    		add_theme_page(esc_html__('Detheme One Click Demo Importer', 'hue-demo'), esc_html__('Import Demo', 'hue-demo'),'manage_options','hue_demo_install', array($this,'demo_install_setting'));
    	}
    }
    ...
    <h1><?php esc_html_e('Detheme Demo Importer','hue-demo');?></h1>
    <?php 
	...
	if($package!=''){
	   ...
	   <?php esc_html_e('import demo content complete','hue-demo');?>
		</div>
		<br/>

	   <a class="button-primary back-demo" href="<?php echo esc_url(admin_url('themes.php?page=hue_demo_install')); ?>"><?php esc_html_e( 'Return to Import Demo', 'hue-demo' ); ?></a>
	
	   <?php

	}
	else{?>
    <h2><?php esc_html_e('WARNING:','hue-demo');?></h2>
    <ol class="install-notice">
    	<li><?php esc_html_e('Recommended for fresh install only','hue-demo');?></li>
    </ol>
<?php
	if (count($packages)):
		...
		?>
    <form id="detheme-demo-form" action="<?php echo admin_url('themes.php?page=hue_demo_install'); ?>" method="post">
...
<?php
    ...
    <h2><?php esc_html_e('Import Options','hue-demo');?></h2>
    <ul class="demo-option">
    	<li><input type="checkbox" name="skip_content" value="1"> <?php esc_html_e( 'Skip content', 'hue-demo' ); ?></li>
    	<li><input type="checkbox" name="skip_menu" value="1"> <?php esc_html_e( 'Skip menu', 'hue-demo' ); ?></li>
    	<li><input type="checkbox" name="skip_theme_option" value="1"> <?php esc_html_e( 'Skip theme option', 'hue-demo' ); ?></li>
    	<li><input type="checkbox" name="skip_sidebar" value="1"> <?php esc_html_e( 'Skip sidebar widget', 'hue-demo' ); ?></li>
    	<li><input type="checkbox" name="skip_attachment" value="1"> <?php esc_html_e( 'Skip media', 'hue-demo' ); ?></li>
<?php if (is_plugin_active('revslider/revslider.php')):?>
    	<li><input type="checkbox" name="skip_slide" value="1"> <?php esc_html_e( 'Skip revolution slider demo', 'hue-demo' ); ?></li>
<?php endif;?>
<?php if (is_plugin_active('LayerSlider/layerslider.php')):?>
    	<li><input type="checkbox" name="skip_layer_slide" value="1"> <?php _e( 'Skip layer slider demo', 'hue-demo' ); ?></li>
<?php endif;?>
<?php if (is_plugin_active('essential-grid/essential-grid.php')):?>
    	<li><input type="checkbox" name="skip_ess" value="1"> <?php esc_html_e( 'Skip Ess. Grid', 'hue-demo' ); ?></li>
<?php endif;?>
<?php if (is_plugin_active('woocommerce/woocommerce.php')):?>
    	<li><input type="checkbox" name="skip_woocommerce" value="1"> <?php esc_html_e( 'Skip woocommerce demo', 'hue-demo' ); ?></li>
<?php endif;?>
    </ul>
         <button type="submit" id="install-demo" class="button-primary"><?php esc_html_e( 'Install Demo', 'hue-demo' ); ?></button>
    </form>
<?php else:?>

	<h2><?php esc_html_e('Demo package not found','hue-demo');?></h2>
	<?php endif;
	}
?>
	</div>
...
<?php
	  ...
	  if($dirlist=$wp_filesystem->dirlist($path)){
	    foreach ($dirlist as $dirname => $dirattr) {
	       ...
	       if($dirattr['type']!='d')
	       	...
	       	if($package= hue_one_click_demo::get_package_info($path.$dirname."/style.css")){
    ...
    function import_ls_slide( $slide ) {
		...
		if(is_array($parsed)) {
			...
			foreach($parsed as $item) {
				...
				print '<div class="updated error">';
				printf( esc_html__( 'Failed to import slide %s', 'hue-demo' ), $name);
				print '</div>';
		}
    ...
    function import_slide( $slide ) {
		...
		if(!$response['success']){
			...
			print '<div class="updated error">';
			printf( esc_html__( 'Failed to import slide %s: %s', 'hue-demo' ), $name,$response['error']);
			print '</div>';
...
$hue_demo =new hue_one_click_demo();
$hue_demo->init();
